...
df = pd.read_csv('../../data/olga.csv', index_col='index')

ids_no_ab = []
ids_ab = []
df['tracks'] = df['tracks'].apply(ast.literal_eval)
for idx, row in df.iterrows():
  ...
  for mbid in row['tracks']:
    ...
    if os.path.isfile(filename):
      ab_ll_found = True
      ids_ab.append(row['musicbrainz_id'])
      break
  if not ab_ll_found:
    ids_no_ab.append(row['musicbrainz_id'])
