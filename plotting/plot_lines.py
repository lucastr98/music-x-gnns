...
if dataset == 'OLGA':
  ...
  job_ids = [(True, [[991490, 1000758, 1000766],
            ...
            (True, [[991529, 1002427, 1002580],
                    ...
                    [1002702, 1002703, 1002704]], 'acousticbrainz, clap')]
else:
  ...
  for j in range(len(job_ids)):
    ...
    for k in range(3):
      with open(f"{path}{str(job_ids[j][1][gnn_layers[i]][k])}.out", "r") as f:
        ...
        ndcgs[cur_feature_idx][i][k] = json_rep['auc']
    cur_feature_idx += 1
scores = np.mean(ndcgs, -1)
stds = np.std(ndcgs, -1)
...
for i in range(len(scores)):
  plt.plot(scores[i], color=colors[i], label=feature_types[i])
# plt.plot(scores, color=colors, label=feature_types)
